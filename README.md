...
Валидация полей :pencil: на клиенте с помощью `yup`.

Автоматическая генерация frontend (`Storybook`) и backend (`Swagger`) документации в режиме разработчика.
...
Есть возможность формировать :whale: `Docker` образы и в последующем на их основании запускать контейнеры. Для этого подготовлены docker-compose скрипты в разделе **Запуск проекта локально**.
...
Формирование dev образов из Dockerfile и развертывание контейнеров на их основании

```bash
  docker compose up --build
```

Формирование prod образов из Dockerfile.prod и развертывание контейнеров на их основании в фоновом режиме

```bash
  docker compose -f docker-compose.prod.yml up --build -d
```

Формирование prod образов из Dockerfile.prod и развертывание контейнеров на их основании, остановка и удаление уже запущенных контейнеров
...
В личном кабинете есть возможность просматривать свою, так и страницы других пользователей (по ссылкам в отзывах на карточке продукта).

![2025-01-30_08-22-27](https://github.com/user-attachments/assets/afb9b104-89ee-42d6-91ad-7b659ff4a18f)

Для соблюдения приватности, есть функция скрытия личной страницы от других пользователей в настройках профиля

![2025-01-30_08-25-12](https://github.com/user-attachments/assets/fd73ad6b-3986-4690-8a5f-88374d28ef4b)
